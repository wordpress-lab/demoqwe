<?php
namespace App\Modules\Manpowerservice\Http\Response;
...
class Manpowerresponse
{
   ...
   public function MakeInvoice($request,$order=null)
  {
      ...
      $invoice_Date = $request->issue_date?$request->issue_date:date("Y-m-d");
   ...
   public function UpdateItemsales($request,$newInvoiceEntry)
  {
   ...
   public function MakeBill($request,$order=null)
  {
      ...
      $bill_date = $request->issue_date?date("Y-m-d",strtotime($request->issue_date)):date("Y-m-d");
   ...
   public function invoiceUpdate($request,$manpower)
   {
       if(!$manpower instanceof Manpower_service)
       {
           throw new \Exception("only Manpower service can store invoice");
       }
       if(!$manpower['invoice_id'])
       {
           return $this->MakeInvoice($request,$manpower);
           throw new \Exception("invoice manpower service update fail");
       }

       return $manpower['invoice_id'];
       //update
...
//       $newInvoice->payment_date= date("d-m-Y",strtotime($payment_date));
   ...
   public function billUpdate($request,$manpower)
   {
       if(!$manpower instanceof Manpower_service)
       {
           throw new \Exception("only manpower can store bill");
       }
       if(!$manpower['bill_id'])
       {
        return $this->MakeBill($request,$manpower);
        throw new \Exception("bill manpower service update fail");
       }

       return $manpower['bill_id'];
       // update
