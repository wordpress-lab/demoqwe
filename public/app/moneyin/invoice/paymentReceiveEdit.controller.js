...
function PaymentReceiveEditController($scope, $http) {
        ...
        .then(function(response){
            ...
            $http
                ...
                .then(function(response){
                    ...
                    $scope.invoices = data;
                    $scope.vat_adjust = $scope.vat_edit_total;
                    $scope.tax_adjust = $scope.tax_edit_total;
                    $scope.other_adjust = $scope.other_edit_total;
    ...
    $scope.ShowAmount = function () {
        ...
        for(var i = 0; i < len; i++)
        {
            ...
            $scope.vat_adjust_des[i]                = $scope.invoices[i].vat_des;
            $scope.invoice_amount_temp[i]           = $scope.invoices[i].payment;
            $scope.tax_adjust_des[i]          = $scope.invoices[i].tax_des;
            $scope.other_adjust_des[i]          = $scope.invoices[i].other_des;
        }
    }
    ...
    $scope.calculateExcessPaymentAmount  =  function () {
        ...
        // for(var i=0;i<len;i++)
        // {
        //     pay_sum = pay_sum+parseFloat($scope.invoice_amount[i]);
        // }
    ...
    $scope.amountReceived = function () {
        ...
        for(var i = 0; i < $scope.invoices.length; i++)
        {
            ...
            var trow_due_amount= parseFloat($scope.invoices[i].payment)+parseFloat($scope.invoices[i].due_amount)+parseFloat($scope.invoices[i].vat_des)+parseFloat($scope.invoices[i].tax_des)+parseFloat($scope.invoices[i].other_des);
